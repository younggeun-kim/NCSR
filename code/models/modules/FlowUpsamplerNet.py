...
class FlowUpsamplerNet(nn.Module):
    def __init__(self, image_shape, hidden_channels, K, L=None,
        ...
        for level in range(1, self.L + 1):
            ...
            self.arch_FlowStep(H, self.K[level], LU_decomposed, W, actnorm_scale, affineInCh, flow_coupling,
                               ...
                               n_conditinal_channels=conditional_channels[level], L_level=level)
            # Split
    ...
    def arch_FlowStep(self, H, K, LU_decomposed, W, actnorm_scale, affineInCh, flow_coupling, flow_permutation,
                      hidden_channels, normOpt, opt, opt_get, n_conditinal_channels=None, L_level=None):
    ...
    def forward(self, gt=None, rrdbResults=None, z=None, epses=None, logdet=0., reverse=False, eps_std=None,
                y_onehot=None, std=None):
   
        if reverse:
            epses_copy = [eps for eps in epses] if isinstance(epses, list) else epses

            sr, logdet = self.decode(rrdbResults, z, eps_std, epses=epses_copy, logdet=logdet, y_onehot=y_onehot, std=std)
            return sr, logdet
        else:
            assert gt is not None
            assert rrdbResults is not None
            z, logdet = self.encode(gt, rrdbResults, logdet=logdet, epses=epses, y_onehot=y_onehot, std=std)
    ...
    def encode(self, gt, rrdbResults, logdet=0.0, epses=None, y_onehot=None, std=None):
        ...
        for layer, shape in zip(self.layers, self.output_shapes):
            ...
            if isinstance(layer, FlowStep):
                fl_fea, logdet = layer(fl_fea, logdet, reverse=reverse, rrdbResults=level_conditionals[level], std=std)
    ...
    def decode(self, rrdbResults, z, eps_std=None, epses=None, logdet=0.0, y_onehot=None, std=None):
        ...
        for layer, shape in zip(reversed(self.layers), reversed(self.output_shapes)):
            ...
            if isinstance(layer, Split2d):
                ...
                fl_fea, logdet = layer(fl_fea, logdet=logdet, reverse=True, rrdbResults=level_conditionals[level], std=std)
            else:
